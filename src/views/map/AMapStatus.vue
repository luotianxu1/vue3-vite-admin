<template>
	<div class="container">
		<div class="btn">
			...
			<el-switch v-model="isShowPoint" active-text="显示点击坐标" inactive-text="不显示点击坐标" />
			...
			<el-select v-model="mapStyle" placeholder="选择图层样式" size="large" @change="changeMapStyle">
				<el-option v-for="item in mapStylesOptions" :key="item.value" :label="item.label" :value="item.value" />
			</el-select>
			...
			<el-switch v-model="isShowCondition" active-text="显示路况" inactive-text="不显示路况" />
			<el-select v-model="mapType" placeholder="选择图层" size="large" @change="changeMapType">
...
const setMapZoomCenter = () => {
	...
	ElMessage.success(`当前层级已设为${zoom}级，中心点已设为 ${lng.toFixed(6)},${lat.toFixed(6)}`)
}
