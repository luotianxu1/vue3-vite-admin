...
const compute = () => {
	...
	// 小圈是否在大圈内
	let isRingInRing = Map.value.GeometryUtil.isRingInRing(polygon2Path, polygon1Path)
	// 两圈是否交叉
	let doesRingRingIntersect = Map.value.GeometryUtil.doesRingRingIntersect(polygon2Path, polygon1Path)
	let ringRingClip = Map.value.GeometryUtil.ringRingClip(polygon2Path, polygon1Path)
