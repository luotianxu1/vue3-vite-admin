    ...
    const initMap = () => {
        AMapLoader.load({
            ...
            .then((AMap) => {
                ...
                let closestPositionOnLine = AMap.GeometryUtil.closestOnLine(
                    data,
                    path
                )
                ...
                let distance = Math.round(
                    AMap.GeometryUtil.distanceToLine(data, path)
                )
                let textPos = [
                    (data.lng + closestPositionOnLine[0]) / 2,
                    (data.lat + closestPositionOnLine[1]) / 2
                ]
