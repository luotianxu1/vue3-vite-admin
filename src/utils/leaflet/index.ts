...
L.GridLayer.include({
	_setZoomTransform: function (level, _center, zoom) {
		...
		let translate = level.origin.multiplyBy(scale).subtract(this._map._getNewPixelOrigin(center, zoom)).round()
	...
	_getTiledPixelBounds: function (_center) {
		...
		let map = this._map
		let mapZoom = map._animatingZoom ? Math.max(map._animateToZoom, map.getZoom()) : map.getZoom()
